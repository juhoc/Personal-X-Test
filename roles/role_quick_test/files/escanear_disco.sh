...
then
        if [ -z "$FS" ]
        then
                echo "La variable <Path_FS> esta vacía"
                echo "Ejemplo: ./escanear_disco.sh despues /path_FS 100"
        elif [ -z "$SZDISK" ]
        then
                ...
                #Guarda el VG del FS que se indico
                #VG=$(df -Th /fsprueba/ | awk '{print $1}' | awk -F/ '{print $4}' | awk -F- '{print $1}')
                VG=$(df -Th $FS | awk '{print $1}' | awk -F/ '{print $4}' | awk -F- '{print $1}')
                #Se exiteinde el VG con el nuevo disco
                vgextend $VG $PDISK
                #(command -v vgextend) $VG $PDISK
                echo "Se extendió "$VG" en el nuevo disco "$PDISK""
