...
if [ $CTMFSC -gt 0 ]; then
    if [ $CTMAG -ge 3 ] && [ $CTMITLC -eq 1 ]; then
        echo "Contro-MAgent,ACTIVO" > /var/opt/ansible/ctm.asb
    elif [ $CTMAG -eq 3 ] && [ $CTMITLC -eq 1 ]; then
        echo "Contro-MAgent,INACTIVO" > /var/opt/ansible/ctm.asb
    fi
else
...
if [ $DMFSC -gt 0 ]; then
    if [ $DMAG -ge 3 ] && [ $DMITLC -eq 1 ]; then
        echo "Dimensions$DMVER,ACTIVO" > /var/opt/ansible/dim.asb
    elif [ $DMAG -eq 0 ] && [ $DMITLC -eq 1 ]; then
        echo "Dimensions$DMVER,INACTIVO" > /var/opt/ansible/dim.asb
    fi
else
